class Drawable {
    ...
    isCollision(element) {
        ...
        return fruit.x1 < player.x2 && player.x1 < fruit.x2 && fruit.y1 < player.y2 && player.y1 < fruit.y2;
    }
    update() {
...
class Player extends Drawable {
    constructor(game) {
        ...
        this.w = 244;
        this.h = 109;
        this.x = window.innerWidth / 2 - this.x / 2;
    ...
    update() {
        ...
        if(this.skillTimer > 240 || (!this.Space && this.skillTimer > 1 )){
            this.couldownTimer++;
...
class Game {
    ...
    loop() {
        ...
        if (time.s1 >= 1 || time.m1 >= 1 || time.m2 >= 1) {
            ...
            $('#playerName').innerHTML = `Печенька, ${this.name}`;
            $('#endTime').innerHTML = `Ваше время: ${time.m1}${time.m2}:${time.s1}${time.s2}`;
            $('#collectedFruits').innerHTML = `Вы собрали: ${this.points} фруктов`;
            $('#congratulation').innerHTML = `Вы проиграли`;
        }
        go('end', 'panel d=flex jusitify-content-center align-items-center')
    }
